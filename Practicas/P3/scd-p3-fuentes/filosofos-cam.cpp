...
const int
    ...
    num_procesos = num_filo_ten + 1,  // número de procesos total filo y ten y camarero
    etiq_sentarse = 1,
    etiq_levantarse = 2,
    id_camarero = 10;
...
void funcion_filosofos(int id)
{
    ...
    // Mensaje a enviar da igual el valor
    int msg = 0;
    while (true)
    {
        ...
        // Solicitar tenedores izquierdo y derecho
        ...
        // Soltar tenedores izquierdo y derecho
        MPI_Ssend(&msg, 1, MPI_INT, id_ten_izq, 0, MPI_COMM_WORLD);
...
{
    int etiq_aceptable = MPI_ANY_TAG, cont_sentados = 0, valor = 0;
    MPI_Status estado;
...
int main(int argc, char **argv)
{
    ...
    if (num_procesos == num_procesos_actual)
    {
        ...
        if (id_propio % 2 == 0 && id_propio != 10) // si es par y no es id 10
            funcion_filosofos(id_propio);          //   es un filósofo
        else if (id_propio % 2 != 0)               // si es impar
            funcion_tenedores(id_propio);          //   es un tenedor
        else                                       // sino, es camarero
            funcion_camarero();
    }
    else
    {
